...
const UploadPage = () => {
  ...
  const handleSubmitGenerateCaption = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/image-processing", {
        ...
        body: JSON.stringify({ prompt, image_url: imageFile[0] }),
      });
      ...
      if (response.status === 429) {
        ...
        notifyRateLimiter();
        setRetryAfter(convertedSecondstoTimeStamp);
  ...
  const notifySuccessCaptionCreation = () =>
    toast.success("Caption successfully created!");

  const notifyFailedCaptionCreation = () =>
    toast.error("Whoops! Our agent might on break. Try again later.");

  const notifyRateLimiter = () =>
    toast.success(
      "We runned out of captions for you for today! Try again later.",
      {
        icon: "⏳",
        duration: 4000,
      }
              ...
              <div className="max-w-md lg:max-w-full mx-auto border border-gray-300 bg-white">
                <header className="flex items-center border-b border-b-gray-300 p-3">
                  ...
                  <div className="flex-grow ml-3 text-sm font-semibold">
                    <a
                      href="https://www.instagram.com/manlio_kt/"
                      target="_blank"
